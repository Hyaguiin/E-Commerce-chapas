...
interface AdminControllProps {
  setCurrentComponent: (component: string) => void; // Tipando a função como recebendo uma string
}

const AdminControll = ({ setCurrentComponent }: AdminControllProps) => {
  const navigate = useNavigate();
  const [active, setActive] = React.useState('home'); // Define o estado inicial como 'home'

  const handleSetActive = (component: string) => {
    setActive(component); // Atualiza o estado 'active' com o nome da página
  ...
  const handleProducts = () => {
    setCurrentComponent('productList'); // Vai para a lista de produtos
    setActive('cart'); // Define 'cart' como ativo ao selecionar "Produtos"
  };

  const handleAddProduct = () => {
    setActive('addProduct'); // Define 'addProduct' como ativo ao selecionar "Adicionar Produto"
  };
    ...
    <div className="flex flex-col w-64 h-screen bg-black text-white shadow-lg">
      ...
      <nav className="flex-1 px-2 py-4">
        <ul>
          ...
          <li className="my-2">
            <button
              className={`flex items-center px-4 py-2 rounded-lg transition duration-200 ${active === 'addProduct' ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
              onClick={handleAddProduct} // Exibe o formulário de adicionar produto
            >
