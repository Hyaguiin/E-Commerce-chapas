...
const CartConfirmation: React.FC<CartConfirmationProps> = ({ orderData, setShowConfirmation }) => {
  return (
    <Dialog open={true} onClose={() => setShowConfirmation(false)} className="relative z-10">
      <DialogPanel className="bg-white p-6 rounded-md shadow-lg text-center space-y-4">
        <DialogTitle className="text-lg font-semibold">Pedido Confirmado!</DialogTitle>
        <p className="text-gray-600">
          Obrigado, <strong>{orderData?.user_name || "Usuário"}</strong>!
          Seu pedido foi realizado com sucesso.
        </p>
        <p className="text-gray-600">
          Total: <strong>R${orderData?.total_amount?.toFixed(2) || "0.00"}</strong>
        </p>
