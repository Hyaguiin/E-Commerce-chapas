...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    if (storedUserId) {
      ...
      (async () => {
        const cart = await fetchCart(storedUserId);
        const cartItemsArray = cart?.data.items || [];
        updateCartItems(cartItemsArray); // Atualizando o contexto com os itens do carrinho
      })();
    }
  }, [updateCartItems]); // Dependência no updateCartItems para garantir que ele seja chamado após o carregamento do usuário
